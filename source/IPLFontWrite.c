...
typedef struct {
	unsigned short font_type, first_char, last_char, subst_char, ascent_units,
			descent_units, widest_char_width, leading_space, cell_width,
			cell_height;
	unsigned long texture_size;
	unsigned short texture_format, texture_columns, texture_rows,
			texture_width, texture_height, offset_charwidth;
	unsigned long offset_tile, size_tile;
} FONT_HEADER;

unsigned int blit_lookup_inv[4] = { COLOR_BLUE, COLOR_GRAY, COLOR_GRAY,
		COLOR_SKYBLUE };
unsigned int blit_lookup[4] = { COLOR_BLUE, COLOR_GRAY, COLOR_GRAY,
		COLOR_SKYBLUE };
unsigned int blit_lookup_norm[4] = { COLOR_WHITE, COLOR_GRAY, COLOR_GRAY,
		COLOR_SKYBLUE };
...
void init_font(void) {
	...
	untile((unsigned char*) &fontFont,
			(unsigned char*) &fontWork[fnt->offset_tile], fnt->texture_width,
			fnt->texture_height);
	...
	for (i = 0; i < 256; ++i) {
		...
		font_offset[i] = (r * fnt->cell_height) * fnt->texture_width + (c
				* fnt->cell_width);
	}
...
int GetTextSizeInPixels(const char *string) {
	...
	for (x = t = 0; t < strlen(string); t++)
		x += font_size[(unsigned int) string[t]];
	return x;
}
