...
static void Initialise() {
	...
	//Wiilight init
	WIILIGHT_Init();
	WIILIGHT_SetLevel(255); //light level
...
void dump_info(char *md5, char *sha1, u32 crc32, int verified) {
	...
	sprintf(infoLine, "--File Generated by CleanRip Mod v%i.%i.%i--"
					  "\r\n\r\nFilename: %s\r\nInternal Name: %s\r\nMD5: %s\r\n"
					  "SHA-1: %s\r\nCRC32: %08X\r\nVersion: 1.0%i\r\nVerified: %s\r\n", 
...
void dump_game(int disc_type, int type, int fs) {
	...
	sprintf(txtbuffer, "Game : %s", &internalName[0]);
	WriteCentre(420, txtbuffer);
	...
	while (!ret && (startLBA + opt_read_size) < endLBA) {
		...
		md5_append(&state, (const md5_byte_t *) buffer, (u32) (opt_read_size
				<< 11));
		// Calculate SHA-1
		SHA1Input(&sha, (const unsigned char *) buffer, (u32) (opt_read_size
				<< 11));
		// Calculate CRC32
		crc32 = Crc32_ComputeBuf( crc32, (const void*) buffer, (u32) (opt_read_size
				<< 11));
